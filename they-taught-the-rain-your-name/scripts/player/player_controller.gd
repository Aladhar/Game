...
@export var stamina_regen_per_second: float = 14.0
@export var stamina_regen_delay: float = 0.85

@export_category("Crouch")
@export var standing_camera_height: float = 1.55
@export var crouching_camera_height: float = 0.95
@export var standing_collider_height: float = 1.65
@export var crouching_collider_height: float = 1.05
...
var was_stamina_ratio: float = -1.0
...
func _apply_movement(delta: float) -> void:
    ...
    var wants_sprint := Input.is_action_pressed("sprint") and not is_crouching and world_dir.length() > 0.0
    var can_sprint := wants_sprint and stamina > 0.5
    ...
    if can_sprint:
        ...
        if stamina_regen_timer <= 0.0:
            stamina = min(max_stamina, stamina + stamina_regen_per_second * delta)
...
func _apply_crouch(delta: float) -> void:
    var camera_height := crouching_camera_height if is_crouching else standing_camera_height
    camera_pivot.position.y = move_toward(camera_pivot.position.y, camera_height, crouch_transition_speed * delta)
...
func _update_crouch_shape(delta: float) -> void:
    ...
    var target_height := crouching_collider_height if is_crouching else standing_collider_height
    var target_y := target_height * 0.5
    if delta >= 1.0:
        capsule.height = target_height
        body_collision.position.y = target_y
    else:
        capsule.height = move_toward(capsule.height, target_height, crouch_transition_speed * delta)
        body_collision.position.y = move_toward(body_collision.position.y, target_y, crouch_transition_speed * delta)
...
func _track_sound_behavior(delta: float) -> void:
    ...
    if Input.is_action_pressed("sprint") and not is_crouching and horizontal_speed > walk_speed and stamina > 0.5:
...
func _emit_stamina_if_changed() -> void:
    var ratio := stamina / max_stamina if max_stamina > 0.0 else 0.0
    if absf(ratio - was_stamina_ratio) < 0.01:
        return
    was_stamina_ratio = ratio
    stamina_changed.emit(stamina, max_stamina)
