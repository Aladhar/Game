"""
Penance Carrier V7 organic clutter pass.

This pass builds on the V6 mood/body pass and corrects:
...
Outputs:
  assets/models/enemies/penance_carrier/blender_work/penance_carrier_end_goal_v7_organic_clutter.blend
  assets/models/enemies/penance_carrier/penance_carrier_end_goal_v7_organic_clutter.glb
"""
...
OUTPUT_BLEND = WORK_DIR / "penance_carrier_end_goal_v7_organic_clutter.blend"
OUTPUT_GLB = ASSET_ROOT / "penance_carrier_end_goal_v7_organic_clutter.glb"
...
def create_materials() -> None:
    MAT["cloth"] = make_mat("PC_V7_wet_black_wrapped_cloth", (0.035, 0.033, 0.030, 1), 0.96)
    MAT["wrap_highlight"] = make_mat("PC_V7_damp_gray_wrap_highlights", (0.13, 0.125, 0.112, 1), 0.94)
    MAT["wood"] = make_mat("PC_V7_rotten_dark_weathered_wood", (0.082, 0.060, 0.041, 1), 0.95)
    MAT["raw_wood"] = make_mat("PC_V7_old_splintered_door_wood", (0.215, 0.180, 0.130, 1), 0.91)
    ...
    MAT["paper"] = make_mat("PC_V7_stained_old_paper", (0.30, 0.255, 0.185, 1), 0.95)
    MAT["wax"] = make_mat("PC_V7_dirty_dead_wax", (0.36, 0.30, 0.21, 1), 0.85)
...
def add_v7_organic_correction() -> None:
    random.seed(808)
    # Break up the flat house read with overlapping rotten planks and dark gaps.
    for name in ["PC_V4_shrine_front_rotten_wall", "PC_V4_shrine_rear_rotten_wall"]:
    ...
    for i, x in enumerate([-0.82, -0.64, -0.47, -0.31, -0.14, 0.02, 0.20, 0.39, 0.58, 0.78]):
        box(
            f"PC_V7_front_rotten_vertical_board_{i:02d}",
            (x + random.uniform(-0.025, 0.025), -0.935, 3.05 + random.uniform(-0.06, 0.08)),
            (random.uniform(0.055, 0.105), 0.030, random.uniform(1.08, 1.54)),
            MAT["raw_wood"] if i in [2, 5, 8] else MAT["wood"],
            rot=(0, 0, math.radians(random.uniform(-4.5, 4.5))),
            collection="PC_V7_organic_clutter",
        )
    for i, (x, z, sx, sz) in enumerate([(-0.55, 3.28, 0.25, 0.42), (-0.13, 3.52, 0.18, 0.24), (0.34, 3.15, 0.24, 0.34), (0.66, 2.78, 0.18, 0.30)]):
        sphere(
            f"PC_V7_front_black_wall_gap_{i:02d}",
            (x, -0.965, z),
            0.14,
            MAT["void"],
            scale=(sx / 0.14, 0.09, sz / 0.14),
            collection="PC_V7_organic_clutter",
        )

    # Replace the toy-ring read with a deeper hood, cloth lips, and a clearer black head void.
    hood = bpy.data.objects.get("PC_V6_head_narrow_hood_oval_ring")
    if hood:
        assign(hood, MAT["cloth"])
        hood.scale.x *= 0.70
        hood.scale.z *= 1.18
    for i, (x, z, rz) in enumerate([(-0.20, 2.62, -8), (-0.13, 2.33, -5), (0.13, 2.34, 5), (0.20, 2.62, 8)]):
        box(
            f"PC_V7_hood_wet_outer_lip_{i:02d}",
            (x, -1.335, z),
            (0.075, 0.045, 0.58),
            MAT["cloth"],
    ...
    sphere("PC_V7_deeper_head_void", (0, -1.36, 2.80), 0.25, MAT["void"], scale=(0.52, 0.055, 1.42), collection="PC_V7_organic_clutter")

    for i, x in enumerate([-0.14, -0.07, 0.0, 0.07, 0.14]):
        box(
            f"PC_V7_door_mask_splinter_{i:02d}",
            (x, -1.365, 1.82 - abs(x) * 0.42),
            (0.040, 0.026, random.uniform(1.08, 1.55)),
            MAT["raw_wood"] if i != 2 else MAT["wood"],
            rot=(0, 0, math.radians(random.uniform(-3, 3))),
            collection="PC_V7_organic_clutter",
        )
    for i, (z, width) in enumerate([(2.18, 0.25), (1.86, 0.22), (1.55, 0.19)]):
        box(f"PC_V7_door_mask_cross_stitch_{i:02d}", (0, -1.398, z), (width, 0.016, 0.022), MAT["metal"], rot=(0, 0, math.radians(-12 if i % 2 else 13)), collection="PC_V7_organic_clutter")

    # Organic wrapped anatomy over the blocky limbs.
    for side, sign in [("left", -1), ("right", 1)]:
        for i, z in enumerate([0.32, 0.44, 0.56, 0.70, 0.86, 1.02, 1.18]):
            box(
                f"PC_V7_{side}_leg_torn_wrap_slash_{i:02d}",
                (sign * (0.31 + random.uniform(-0.035, 0.035)), -0.40, z),
                (0.27, 0.032, 0.030),
                MAT["wrap_highlight"] if i % 3 == 0 else MAT["cloth"],
                rot=(0, 0, math.radians(sign * random.uniform(18, 34))),
                collection="PC_V7_organic_clutter",
            )
        for i, z in enumerate([1.22, 1.38, 1.55, 1.74, 1.92, 2.08]):
            box(
                f"PC_V7_{side}_arm_dirty_bandage_{i:02d}",
                (sign * (0.64 + random.uniform(-0.045, 0.030)), -0.82, z),
                (0.24, 0.030, 0.030),
                MAT["wrap_highlight"] if i % 4 == 0 else MAT["cloth"],
                rot=(0, 0, math.radians(sign * random.uniform(-34, -15))),
                collection="PC_V7_organic_clutter",
            )

    # Dense hanging silhouette: chains, cords, keys, bells, and cloth tassels.
    anchors = [-0.96, -0.82, -0.68, -0.52, -0.36, -0.21, -0.06, 0.10, 0.27, 0.44, 0.61, 0.78, 0.94]
    for i, x in enumerate(anchors):
        z0 = random.uniform(3.34, 3.86)
        end = chain_links(f"PC_V7_front_deep_chain_{i:02d}", (x, -1.02, z0), random.randint(8, 15), random.uniform(0.075, 0.115), MAT["metal"], random.uniform(0.55, 0.92), random.random() * 6)
        if i % 3 == 0:
            bell_proxy(f"PC_V7_front_low_bell_{i:02d}", (end[0], end[1] - 0.02, end[2] - 0.07), random.uniform(0.55, 0.95))
        elif i % 3 == 1:
            box(f"PC_V7_front_hanging_key_{i:02d}", (end[0], end[1] - 0.02, end[2] - 0.10), (0.025, 0.014, 0.20), MAT["brass"], collection="PC_V7_organic_clutter")
            torus(f"PC_V7_front_key_ring_{i:02d}", (end[0], end[1] - 0.02, end[2] + 0.02), 0.038, 0.004, MAT["brass"], rot=(math.radians(90), 0, 0), collection="PC_V7_organic_clutter")
        else:
            box(f"PC_V7_front_tar_cloth_tassel_{i:02d}", (end[0], end[1] - 0.01, end[2] - 0.18), (0.045, 0.022, random.uniform(0.34, 0.68)), MAT["cloth"], rot=(0, 0, math.radians(random.uniform(-6, 6))), collection="PC_V7_organic_clutter")

    # Back needs disgusting mass, but still visible structure instead of one flat black slab.
    for i, x in enumerate([-0.72, -0.55, -0.38, -0.22, -0.05, 0.13, 0.31, 0.50, 0.70]):
        box(
            f"PC_V7_back_wet_rotten_rib_{i:02d}",
            (x, 1.075, 2.42 + random.uniform(-0.10, 0.16)),
            (random.uniform(0.055, 0.095), 0.040, random.uniform(1.12, 1.78)),
            MAT["cloth"] if i % 2 else MAT["wood"],
            rot=(0, 0, math.radians(random.uniform(-8, 8))),
            collection="PC_V7_organic_clutter",
        )
    for i, (x, z) in enumerate([(-0.50, 3.22), (-0.18, 2.92), (0.12, 3.36), (0.43, 2.75), (0.02, 2.42)]):
        sphere(f"PC_V7_back_deep_wound_void_{i:02d}", (x, 1.105, z), 0.16, MAT["void"], scale=(1.15, 0.08, 1.55), collection="PC_V7_organic_clutter")
...
def main() -> None:
    print("Penance Carrier V7 organic clutter pass starting...", flush=True)
    random.seed(1707)
    clear_scene()
    create_materials()
