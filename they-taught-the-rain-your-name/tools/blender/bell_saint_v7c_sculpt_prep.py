# Bell Saint v7c Blender Sculpt-Prep Kit
# Fixes:
# 1. No Windows path examples inside normal docstrings, avoiding unicodeescape errors.
# 2. Materials are created after scene cleanup/import, avoiding removed Material references.
#
# If automatic project detection fails, set USER_PROJECT_ROOT to the folder that contains project.godot.
# Example:
...
def likely_project_paths() -> list[Path]:
    home = Path.home()
    candidates = [
    ...
    for base in [
        ...
        if base.exists():
            try:
                candidates.extend(base.glob("*/they-taught-the-rain-your-name"))
                candidates.extend(base.glob("*/*/they-taught-the-rain-your-name"))
            except Exception:
                pass
...
def find_project_root() -> Path:
    ...
    checks.extend(likely_project_paths())
    ...
    raise FileNotFoundError(
        "Could not find the Godot project root containing project.godot.\n"
        "Set USER_PROJECT_ROOT at the top of this script or run Blender from the Godot project root.\n\n"
        "Attempted paths:\n" + attempted
    )
...
OUTPUT_BLEND = WORK_DIR / "bell_saint_v7c_sculpt_prep.blend"
OUTPUT_GLB = PROJECT_ROOT / "assets/models/enemies/bell_saint/bell_saint_v7c_sculpt_prep.glb"


# Material globals are assigned after cleanup/import.
MAT_WET_CLOTH = None
MAT_CLOTH_HI = None
MAT_MUD = None
MAT_BRONZE = None
MAT_BRONZE_EDGE = None
MAT_PATINA = None
MAT_RUST = None
MAT_IRON = None
MAT_ROPE = None
...
def clear_scene() -> None:
    ...
    # Only purge orphan data. Do not create materials before this.
...
def create_all_materials() -> None:
    global MAT_WET_CLOTH
    global MAT_CLOTH_HI
    global MAT_MUD
    global MAT_BRONZE
    global MAT_BRONZE_EDGE
    global MAT_PATINA
    global MAT_RUST
    global MAT_IRON
    global MAT_ROPE
...
def add_weighted_normals(obj: bpy.types.Object) -> None:
    ...
    try:
        ...
        print("Skipped weighted normals for", obj.name, exc)
...
def add_bevel(obj: bpy.types.Object, width: float, segments: int) -> None:
    ...
    try:
        ...
        print("Skipped bevel for", obj.name, exc)
...
def add_noise_displace(obj: bpy.types.Object, name: str, strength: float, size: float) -> None:
    ...
    try:
        ...
        print("Skipped noise displacement for", obj.name, exc)
...
def add_subdivision(obj: bpy.types.Object, levels: int = 1) -> None:
    ...
    try:
        ...
        print("Skipped subdivision for", obj.name, exc)
...
def assign_materials_and_collections() -> None:
    ensure_collection("BS7_01_Body_Cloth_Sculpt")
    ensure_collection("BS7_02_Bell_Head_Metal_Sculpt")
    ensure_collection("BS7_03_Back_Frame_Chains")
    ensure_collection("BS7_04_Decals_Patina_Rust_Wax")
...
def add_preview_scene() -> None:
    ...
    def marker(name: str, loc, color) -> None:
