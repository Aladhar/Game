"""
Penance Carrier V10 slim wrapped-body pass.
...
Outputs:
  assets/models/enemies/penance_carrier/blender_work/penance_carrier_end_goal_v10_slim_wrapped_body.blend
  assets/models/enemies/penance_carrier/penance_carrier_end_goal_v10_slim_wrapped_body.glb
"""
...
OUTPUT_BLEND = WORK_DIR / "penance_carrier_end_goal_v10_slim_wrapped_body.blend"
OUTPUT_GLB = ASSET_ROOT / "penance_carrier_end_goal_v10_slim_wrapped_body.glb"
...
def add_v10_slim_wrapped_body_refinement() -> None:
    random.seed(1110)

    # V9 removed gaps but got too bulky. Keep the nested fill, then pull it back into
    # a gaunt, hunched shape closer to the reference.
    scale_overrides = {
        "PC_V9_inner_hunched_torso_core": (0.70, 0.92, 0.90),
        "PC_V9_upper_chest_under_mask_fill": (0.74, 0.90, 0.84),
        "PC_V9_lower_belly_sagging_fill": (0.58, 0.88, 0.78),
        "PC_V9_left_hip_cloth_fill": (0.56, 0.88, 0.78),
        "PC_V9_right_hip_cloth_fill": (0.56, 0.88, 0.78),
    }
    for name, mult in scale_overrides.items():
    ...
    for side in ["left", "right"]:
        for part in ["upper_arm_filled_muscle", "forearm_filled_muscle", "thigh_filled_column", "shin_filled_column"]:
            obj = bpy.data.objects.get(f"PC_V9_{side}_{part}")
            if obj:
                obj.scale.x *= 0.72
                obj.scale.y *= 0.92
                obj.location.y -= 0.06

    # Dark nested crease shapes on top of the fill make the body read as wrapped,
    # not as one uninterrupted balloon.
    for i, (x, z, h) in enumerate([(-0.18, 1.75, 1.08), (-0.09, 1.62, 1.28), (0.00, 1.52, 1.34), (0.09, 1.62, 1.22), (0.18, 1.72, 0.98)]):
        box(
            f"PC_V10_body_vertical_dark_crease_{i:02d}",
            (x, -1.075, z),
            (0.030, 0.026, h),
            MAT["void"] if i == 2 else MAT["cloth"],
            rot=(0, 0, math.radians(random.uniform(-2.5, 2.5))),
            collection="PC_V10_slim_wrapped_body",
        )

    for i, z in enumerate([0.94, 1.10, 1.27, 1.45, 1.64, 1.84, 2.02]):
        box(
            f"PC_V10_torso_cross_wrap_{i:02d}",
            (random.uniform(-0.025, 0.025), -1.105, z),
            (0.46 - min(i, 4) * 0.035, 0.024, 0.026),
            MAT["wrap_highlight"] if i % 3 == 0 else MAT["cloth"],
            rot=(0, 0, math.radians(random.choice([-1, 1]) * random.uniform(9, 19))),
            collection="PC_V10_slim_wrapped_body",
        )

    # Add inner calf/thigh silhouettes so the legs are connected but still thin.
    for side, sign in [("left", -1), ("right", 1)]:
        for i, (z, w) in enumerate([(0.30, 0.17), (0.48, 0.19), (0.68, 0.20), (0.88, 0.21), (1.08, 0.20)]):
            box(
                f"PC_V10_{side}_leg_inner_shadow_{i:02d}",
                (sign * 0.235, -0.585, z),
                (w, 0.030, 0.034),
                MAT["void"] if i % 2 == 0 else MAT["cloth"],
                rot=(0, 0, math.radians(sign * random.uniform(12, 24))),
                collection="PC_V10_slim_wrapped_body",
            )
        for i, z in enumerate([1.26, 1.43, 1.61, 1.80, 1.98]):
            box(
                f"PC_V10_{side}_arm_inner_shadow_{i:02d}",
                (sign * 0.585, -0.965, z),
                (0.17, 0.026, 0.026),
                MAT["void"] if i in [1, 3] else MAT["cloth"],
                rot=(0, 0, math.radians(sign * random.uniform(-28, -14))),
                collection="PC_V10_slim_wrapped_body",
            )

    # The front mask should hide enough body to feel like the door-face in the PNG.
    for i, (x, h, z) in enumerate([(-0.12, 1.52, 1.78), (-0.04, 1.68, 1.82), (0.04, 1.64, 1.80), (0.12, 1.46, 1.74)]):
        box(
            f"PC_V10_door_face_overlay_plank_{i:02d}",
            (x, -1.505, z),
            (0.050, 0.018, h),
            MAT["raw_wood"],
            rot=(0, 0, math.radians(random.uniform(-1.6, 1.6))),
            collection="PC_V10_slim_wrapped_body",
        )
...
def main() -> None:
    print("Penance Carrier V10 slim wrapped-body pass starting...", flush=True)
    random.seed(2010)
    clear_scene()
    create_materials()
    ...
    add_v10_slim_wrapped_body_refinement()
